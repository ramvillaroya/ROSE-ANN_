...
// Include DB config
@include 'config.php'; // Make sure config.php defines $conn

if ($_SERVER["REQUEST_METHOD"] == "POST") {
    $usertype  = $_POST['usertype']; // 'admin' or 'client'
    ...
    $role = $usertype ?? 'user'; // Default to 'user'

    // Insert new user with all fields
    $insert = $conn->prepare(
        "INSERT INTO users 
            (email, password, firstname, lastname, address, role) 
         VALUES 
            (?, ?, ?, ?, ?, ?)"
    );
    if (!$insert) {
